...
const ProfileModal = ({ closeModal }) => {
    ...
    const changeData = async (type, inputData) => {
        const endPoint = `http://localhost:5500/contact/update/${type}`;
        if (!inputData) {
    ...
    const handleProfileUpdate = async (e) => {
        ...
        const endPoint = "http://localhost:5500/contact/update/profile";

        try {
            fromData.set("profile", file);
            const responce = await axios.put(endPoint, fromData, {
                headers: {
                    Authorization: `Bearer ${getCookie("token")}`,
                },
            });
